...
def name_to_key(name):
    """Convert substation name to a valid TS identifier (strip spaces and apostrophes)."""
    return name.replace("'", "").replace(" ", "")


def compile_lookups(case_name):
    case_dir = DATA_DIR / case_name
    source_dir = case_dir / "source"
    output_file = case_dir / "lookups.ts"

    subs = parse_csv(source_dir / "substations.csv")
    branches = parse_csv(source_dir / "branches.csv")

    sub_lines = "\n".join(
        f'    {name_to_key(row["Name"])}: "{row["Number"]}",'
        for row in subs
    )
    line_lines = "\n".join(
        f'    {name_to_key(row["FromSub"])}_{name_to_key(row["ToSub"])}: "{row["Number"]}",'
        for row in branches
    )

    output = f"""// Auto-generated by compile.py — do not edit manually
...
"""
    ...
    with open(output_file, "w", encoding="utf-8", newline="\n") as f:
        f.write(output)
    print(f"Generated {output_file} ({len(subs)} substations, {len(branches)} branches)")


for case in CASES:
    compile_case(case)
    compile_lookups(case)
