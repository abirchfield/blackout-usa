"""Prebuild: JSON source → _out/ (lookups.ts, grid-data.json, registry.ts)"""
...
def build_subs(rows):
    ...
    for i, raw in enumerate(rows):
        ...
        sub = {
            "Number": num, "Name": raw["Name"],
            "Latitude": float(raw["Latitude"]), "Longitude": float(raw["Longitude"]),
            "Category": cat, "Units": len(units), "U": units,
            "Pmax": tpmax, "Pmin": tpmin,
            "idx": int(num) - 1, "pmax": tpmax / n, "pmin": tpmin / n,
            "isLoad": cat == "Load", "isRenewable": cat in ("Wind", "Solar PV"),
        }
        for f in PARAMS[2:]:
...
def build_branches(rows, subs):
    ...
    for raw in rows:
        ...
        branches[bn] = {
            "Number": bn, "FromSub": raw["FromBus"], "ToSub": raw["ToBus"],
            "FromNum": fn, "ToNum": tn, "Z": z, "Pmax": float(raw["Pmax"]),
            "P": 0, "Status": "IN",
            "fromIdx": int(fn) - 1, "toIdx": int(tn) - 1, "ybr": -1.0 / z, "dist": dist,
        }
...
def compile_lookups(case_name, subs, branches):
    sub_lines = "\n".join(
        f'    {key(r["Name"])}: "{r.get("Number", i + 1)}",' for i, r in enumerate(subs))

    seen: dict[str, int] = {}
    line_entries = []
    for r in branches:
        bk = f'{key(r["FromBus"])}_{key(r["ToBus"])}'
        c = seen.get(bk, 0); seen[bk] = c + 1
        line_entries.append(f'    {bk if c == 0 else f"{bk}_{c + 1}"}: "{r["Number"]}",')

    out = OUT_DIR / case_name
    out.mkdir(parents=True, exist_ok=True)
    (out / "lookups.ts").write_text(f"""// Auto-generated by runme.py — do not edit manually
...
def compile_grid_data(case_name, case_dir, subs_rows, branch_rows):
    ...
    with open(out / "grid-data.json", "w", encoding="utf-8", newline="\n") as f:
        json.dump({"subs": subs, "branches": build_branches(branch_rows, subs),
                   "borders": borders, "nsubs": len(subs_rows)}, f, separators=(",", ":"))


def compile_scenarios_template(case_name, case_dir):
    dest = case_dir / "scenarios.ts"
    if dest.exists():
        return False
...
def compile_registry(cases):
    loaders = "\n".join(
        f"    '{n}': () => Promise.all([\n"
        f"        import('./{n}/grid-data.json'),\n"
        f"        import('../{n}/case.json'),\n"
        f"        import('../{n}/scenarios'),\n"
        f"    ]).then(([{{ default: gridData }}, {{ default: caseConfig }}, {{ scenarios }}]) =>\n"
        f"        ({{ ...caseConfig, gridData, scenarios }}) as GridCase\n"
        f"    )," for n in cases)
    default = cases[0] if cases else "texas"

    OUT_DIR.mkdir(exist_ok=True)
    (OUT_DIR / "registry.ts").write_text(f"""// Auto-generated by runme.py — do not edit manually
import type {{ GridCase }} from "$lib/types";

const loaders: Record<string, () => Promise<GridCase>> = {{
{loaders}
}};

export const CASE_NAMES = Object.keys(loaders);
export const DEFAULT_CASE = '{default}';

export async function loadCase(name: string): Promise<GridCase> {{
    const loader = loaders[name];
    if (!loader) throw new Error(`Unknown case: ${{name}}. Available: ${{CASE_NAMES.join(', ')}}`);
    return loader();
}}
""", encoding="utf-8", newline="\n")
...
def main():
    ...
    if not cases:
        print("[prebuild] No case folders found"); return

    for name in cases:
        d = DATA_DIR / name
        if compile_scenarios_template(name, d):
            print(f"[prebuild] {name}: generated template scenarios.ts")
        ...
        compile_grid_data(name, d, subs, branches)
        print(f"[prebuild] {name}: {len(subs)} subs, {len(branches)} branches")

    compile_registry(cases)
    print(f"[prebuild] registry.ts ({len(cases)} case(s))")
