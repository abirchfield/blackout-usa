 ...
 */
export function solvePowerFlow(state: GameState, alpha: number) {
  // 1. Update Unit P (Injection) based on Dispatch Alpha
  const pvec = calculatePowerInjectionVector(state, alpha);
...
function calculatePowerInjectionVector(state: GameState, alpha: number): math.Matrix {
...
function rebuildYbus(state: GameState) {
  ...
  for (const key in state.branches) {
    ...
    if (br.Status1 === BranchStatus.IN) {
      addAdmittance(state.Ybus, i, j, ybr);
    }
...
function updateBranchFlows(state: GameState, theta: math.Matrix) {
  for (const key in state.branches) {
    const br = state.branches[key];
    const ybr = -1 / br.Z;
    const i = parseInt(br.FromNum) - 1;
