...
export const ForecastChart = memo(function ForecastChart({ forecast, progress }: ForecastChartProps) {
  ...
  useEffect(() => {
    ...
    if (!colorsRef.current) {
      colorsRef.current = resolveChartColors();
    }

    drawForecastChart(ctx, w, h, forecast, progress, colorsRef.current);
  }, [forecast, progress, resolvedTheme]);

  return (
    <div ref={containerRef} className="h-[44px] sm:h-[52px] w-full">
